...
const Applications = () => {
  ...
  const handleDeleteApplication=async(id)=>{
    try {
      ...
      const response = await axios.delete(`http://localhost:8000/application/delete/${id}`, {withCredentials: true});
  ...
  const fetchAllJobApplicationOfThisJob=async()=>{
    try {
      ...
      const response = await axios.get("http://localhost:8000/application/employer/getAll", {withCredentials: true});
      if(response.data.success){
