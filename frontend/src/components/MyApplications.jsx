...
const MyApplications=() => {
  ...
  const fetchUserAllJobApplication=async()=>{
    try {
      ...
      const response = await axios.get("http://localhost:8000/application/jobSeeker/getAll", {withCredentials: true});
  ...
  const handleDeleteApplication=async(jobId) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:8000/application/delete/${jobId}`, {withCredentials: true});
      if(response.data.success){
