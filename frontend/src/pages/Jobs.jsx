...
const Jobs = () => {
  ...
  const fetchAllJobs = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/job/getAll?searchKeyword=${searchKeyword}&city=${selectedCity}&category=${jobCategory}`, { withCredentials: true, headers: { "Content-Type": "application/json" } });
      ...
      {loading ? (
        ...
        <div className="job_outer_container w-full h-auto flex justify-center items-center">
          <section className="jobs w-[95%] h-auto container flex flex-col justify-center items-center mx-auto">
            <div className="search-tab-wrapper w-[60%] border border-gray-400 shadow-md my-6 flex justify-between items-center rounded-md">
              ...
              />
              <button className="w-[20%] p-3 bg-green-400 text-white flex justify-center items-center gap-3 font-semibold" onClick={handleSearch}>Find Here<FaSearch /></button>
