...
const PostApplication = () => {
  ...
  const checkAlreadyAppliedOrNot = async () => {
    try {
      const response = await axios.get(`http://localhost:8000/application/applied/${jobId}`, { withCredentials: true });
  ...
  const fetchSinglJob = async () => {
    try {
      ...
      const response = await axios.get(`http://localhost:8000/job/get/${jobId}`, { withCredentials: true });
      if (response.data.success) {
  ...
  const handlePostApplication = async (e) => {
    ...
    if (!postApplicationInput.resume || !postApplicationInput.coverLetter) {
      ...
      try {
        ...
        const response = await axios.post(`http://localhost:8000/application/post/${jobId}`, formData, { headers: { "Content-Type": "multipart/form-data" }, withCredentials: true });
        if (response.data.success) {
      ...
      <article className="application_page w-[95%] h-auto container mx-auto grid grid-cols-2 gap-6">
        ...
        <div className="job-details text-zinc-600">
          <header className="my-2">
            ...
            <p className="text-sm">Rs. {singleJob?.salary} a month</p>
          </header>
